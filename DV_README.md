...
| Timer periódico (`Global.asax.cs`, cada N min) | ✅ `VerificarTablaCritica<BE_Usuario>` (liviana) | |
...
2. **Timer periódico**: cada `IntervaloVerificacionMinutos` (`Web.config`, default 5 si falta/inválido), corre `VerificarTablaCritica<BE_Usuario>()` en background (`System.Threading.Timer`). Si falla con una excepción, no la deja escapar (mataría el proceso), pero deja registro en `App_Data\dv-verificacion-periodica.log` antes de descartarla — para que la falla no pase desapercibida. Protegido contra solapamiento consigo mismo (`Interlocked`).
3. **Login (`BLL_Usuario.Login`)**: NO verifica integridad. Valida credenciales, obtiene rol, devuelve el usuario. El caller (`LoginIniciarSesion.aspx.cs`) setea `Session["Usuario"]` y redirige a la home según rol, exactamente igual esté el sistema bloqueado o no.
...
// Cada N minutos (timer en background, ver Global.asax.cs):
SERVICIOS.SERVICIOS_DigitoVerificador.VerificarTablaCritica<BE.BE_Usuario>();
```
...
| Arranque de la app | `VerificarIntegridadTotal(null)` |
| Chequeo periódico liviano en background | `VerificarTablaCritica<BE_Usuario>()` |
